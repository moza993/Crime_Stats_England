...
constabulary = st.selectbox("Select a Constabulary:", sorted(map_df['Constabulary'].dropna().unique()))
...
# --- Show filtered count ---
st.markdown(f"Showing **{len(filtered)}** `{crime_type}` crimes in `{constabulary}` during `{month}`")
